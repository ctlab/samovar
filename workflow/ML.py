...
def process_sample(sample_file, output_dir, model=None, classify_unclassified=False, features_df=None):
...
if args.features and os.path.exists(args.features):
    ...
    print(f"[INFO] Merged features: columns increased from {initial_shape[1]} to {validation_df.shape[1]}")
...
best_model, models, metrics, X_test_processed, y_test_processed = train_models(validation_df)
