...
parser.add_argument(
    "--true_annotation", "-t", type=str, required=False, default="(?<=taxid:)[0-9]*"
)
parser.add_argument("--split_sample_name", "-s", type=int, required=False, default=1)
parser.add_argument(
    "--nodes_dmp", type=str, required=False, default="tests_outs/kaiju_db/nodes.dmp"
)
...
raw_list = [
    ...
    if os.path.isfile(os.path.join(args.input_dir, f)) and f.endswith(".out")
]
