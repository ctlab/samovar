samovar <- function(
    ...
    sample_amount = 30, #number of samples
    ...
    initial = F, #initialized sp
    initial_level = F, # initialized level
    generate_initial = T, 
    generated_amount = 25, #number of generated samples
    normalization_function = function(x) log10(1/(1-log10(x+1)))*(1-2*log10(x+1))
) {
  
  # Setup ----
  
  # Add libraries ----
  cat("\n--------\nInitiallized for:", mesh_id, "\n")
  liblist <- c("tidyverse",
               "corrplot",
               "viridis",
               "ape",
  ...
  if (!isFALSE(samovar_table)) {
    res_sp <- samovar_table
  } else {
  ...
  # Deal with initials ----
  if (isFALSE(initial)) initial <- sample(rownames(res_sp_scale), 1)
  if (isFALSE(initial_level)) {
    initial_level <-res_sp[rownames(res_sp) == initial,]
    initial_level <- runif(1,0, max(initial_level))
  }
  ...
  for (i in 1:generated_amount) {
    ...
    # Deal with initials ----
    if (generated_amount) {
      initial <- sample(rownames(res_sp_scale), 1)
    ...
    res$res_scale[res$res_scale < trsh] <- 0
    if(sum(res$res_scale, na.rm = T) > 1 ) res$res_scale = res$res_scale / sum (res$res_scale)
  ...
  data_generated[data_generated < trsh] <- 0
  data_generated[data_generated %>% is.na] <- 0
  data_generated <- data_generated[,
                                   apply(data_generated, 2, sum) > 0] %>% 
