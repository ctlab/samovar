...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Extract features from FASTQ files safely using Two-Pass chunking.")
    parser.add_argument("fastq", help="Input FASTQ file")
    parser.add_argument("-o", "--output", default="features.tsv", help="Output TSV file")
    ...
    parser.add_argument("--chunk_size", type=int, default=50000, help="Number of reads to process in RAM at once")
    args = parser.parse_args()

    # Initialize online models for dimensionality reduction and clustering
    ipca = IncrementalPCA(n_components=5)
    mbk = MiniBatchKMeans(n_clusters=args.clusters, random_state=42, batch_size=args.chunk_size, n_init='auto')
    ...
    # ---------------------------------------------------------
    print(f"[INFO] PASS 1: Training models on {args.fastq} in chunks of {args.chunk_size} reads...")
    kmer_matrix = []
    processed_count = 0
