...
def _fit_or_partial(ipca, mbk, k_mat, n_pca, args):
    """Create or update IncrementalPCA / MiniBatchKMeans for this chunk."""
    n_samples, n_features = k_mat.shape
    n_pca = max(1, min(n_pca, n_samples, n_features))
    n_clusters = max(1, min(args.clusters, n_samples))
    if ipca is None:
    ...
    return ipca, mbk, n_pca


def _add_cluster_features(df, k_mat, ipca, mbk, n_pca):
    """Attach k-mer PCA and cluster labels; no-op if models were not fitted."""
    if ipca is None or mbk is None or k_mat.size == 0:
        df["tetrabin"] = 0
        return df
...
def main():
    ...
    # Fit after the first chunk so n_components cannot exceed n_samples.
    ipca = None
    mbk = None
    n_pca = 5
    ...
    for _, seq, _ in read_fastq(args.fastq):
        ...
        if len(kmer_matrix) >= args.chunk_size:
            ...
            ipca, mbk, n_pca = _fit_or_partial(ipca, mbk, k_mat, n_pca, args)
            kmer_matrix = []
    ...
    for rid, seq, qual in read_fastq(args.fastq):
        ...
        if len(records) >= args.chunk_size:
            ...
            _add_cluster_features(df, k_mat, ipca, mbk, n_pca)
