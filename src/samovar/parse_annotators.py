...
class Annotation:
    ...
    def __init__(
        self,
        file_path_type: Dict[str, str],
        nodes_dmp_path: str,
        get_true_annotation: Optional[str] = None,
    ):
        ...
        for path, tool_type in file_path_type.items():
            ...
            try:
                ...
                if os.path.getsize(path) == 0:
                    print(
                        f"[WARNING] File is empty: {path}. Emulating unclassified predictions."
                    )
                ...
                if df.empty and os.path.getsize(path) != 0:
                    print(
                        f"[WARNING] No data found in {path}. Treating entire sample as unclassified for this tool."
                    )
        ...
        if get_true_annotation is not None:
            ...
            print(
                f"Extracted {len([x for x in self.true_annotation if x])} ground truth annotations."
            )
        ...
        self.rank_list = self.list2set(
            [*self.annotation_list, *self.true_annotation_list]
        )
    ...
    def correct_annotations(self, rank: str = "species") -> pd.DataFrame:
        ...
        return pd.DataFrame(
            self.rank_annotation(rank).correct_annotation().annotation.value_counts()
        )
    ...
    def export(self, file: Optional[str] = None) -> pd.DataFrame:
        ...
        df_return = self.DataFrame.loc[
            :, [col for col in self.DataFrame if col.startswith("taxID")]
        ]
...
class RankAnnotation:
    ...
    def make(
        self, annotation: pd.DataFrame, rank_dict: Dict[str, str]
    ) -> "RankAnnotation":
...
if __name__ == "__main__":
    ...
    if not out_files:
        print(
            f"[ERROR] No .out files found in {results_dir}. Did Snakemake finish successfully?"
        )
        sys.exit(1)
