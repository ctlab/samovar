#!/usr/bin/env bash
# Phage generate/build/reindex demo (NCBI assemblies + named phage_test DBs).
#
# Phases (SAMOVAR_PHASE, default all):
#   database1  — download generate genomes into $out/.genomes, build kaiju/kraken2
#                as phage_test, then `samovar generate --reindex 1`
#   database2  — `samovar generate --reindex 0` with indexed phage_test DBs
#   reindex    — `samovar reindex` on database2 (move processed → samovar_database)
#
# Overridable:
#   SAMOVAR_OUTDIR_1  default samovar/database1
#   SAMOVAR_OUTDIR_2  default samovar/database2
#   N_SAMPLES, TOTAL_READS
set -euo pipefail
...
out1="${SAMOVAR_OUTDIR_1:-samovar/database1}"
out2="${SAMOVAR_OUTDIR_2:-samovar/database2}"
n_samples="${N_SAMPLES:-2}"
total_reads="${TOTAL_READS:-200}"
...
indexed_path() {
  python - <<'PY' "$1"
import sys
from samovar.genome_index import resolve_indexed_path
p = resolve_indexed_path(sys.argv[1])
print(p or "")
PY
}
...
phase_database1() {
  ...
  echo "==> Download generate accessions into ${out1} (.genomes, not indexed)"
  ...
  echo "==> generate --reindex 1 (store + index; stage copies in ${out1}/.genomes)"
  samovar generate \
    --accessions "${GEN1[@]}" \
    --reindex 1 \
    --raw-genomes 0 \
...
phase_database2() {
  ...
  echo "==> generate --reindex 0 with phage_test DBs (skip already indexed)"
  samovar generate \
    --accessions "${GEN2[@]}" \
    --reindex 0 \
    --raw-genomes 0 \
...
# New download(s) must sit in the run folder; already-indexed accessions are staged there too.
...
PY
  ...
  samovar exec --output_dir "$out2"
}

phase_reindex() {
  echo "==> samovar reindex ${out2} (processed only → samovar_database)"
  before="$(python - <<'PY'
