...
echo "==================================================="
echo "  SamovaR: Complex Mock Community Generator (40x)"
echo "==================================================="

# Directories
MOCK_DIR="genomes/mock_community"
OUT_DIR="tests_outs/benchmarking/initial"
mkdir -p $MOCK_DIR
mkdir -p $OUT_DIR
...
for ACC in "${ALL_GENOMES[@]}"; do
    if [ ! -s "${ACC}.fa" ]; then
        ...
        wget -qO "${ACC}.fa" "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&id=${ACC}&rettype=fasta&retmode=text"
        sleep 1
    fi
done
cd ../../
...
# --- 3. Generate Reads ---
echo "[INFO] Generating 5 Million paired-end reads..."

iss generate \
    --genomes genomes/mock_community/*.fa \
    --model hiseq \
    --n_reads 5000000 \
    --cpus 16 \
    --output tests_outs/benchmarking/initial/complex_mock